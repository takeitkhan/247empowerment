...
<head>
    <meta charset="UTF-8" />
    <title><?= esc_html(get_the_title($event)); ?> - Shareable</title>

    <link rel="canonical" href="<?= esc_url($custom_permalink . 'shareable/'); ?>" />

    <!-- Open Graph meta tags -->
    <meta property="og:title" content="<?= esc_attr(get_the_title($event)); ?>" />
    ...
    <meta name="twitter:title" content="<?= esc_attr(get_the_title($event)); ?>" />
...
<body>
    ...
    <h1><?= esc_html(get_the_title($event)); ?></h1>

    <img src="<?= esc_url($thumbnail_url); ?>" alt="<?= esc_attr(get_the_title($event)); ?>" />

    <?php if ($instructor): ?>
        <p><strong>Organizer:</strong> <?= esc_html($instructor); ?></p>
    <?php endif; ?>

    <?php if ($duration): ?>
        <p><strong>Date/Duration:</strong> <?= esc_html($duration); ?></p>
    <?php endif; ?>
    ...
    <p><em>Full event details available after login.</em></p>
